...
if (extract(HOUR from timediff(endTime,startTime)) < 1 and extract(MINUTE from timediff(endTime,startTime)) < (select minimal_timeslot from sport_centres where (centre_id like pcentre and city_id like pcity))) then
	set return_message := "Less than minimal timeslot";
    select return_message;
    leave proc;
end if;

if (extract(HOUR from timediff(endTime,startTime)) < 1 and extract(MINUTE from timediff(endTime,startTime)) > (select maximum from sport_centres where (centre_id like pcentre and city_id like pcity))) then
	set return_message := "More than maximum timeslot";
    select return_message;
    leave proc;
end if;
